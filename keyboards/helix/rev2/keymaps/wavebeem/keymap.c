...
void oled_draw_stuff(int t) {
    static int h = OLED_DISPLAY_HEIGHT;
    static int w = OLED_DISPLAY_WIDTH;
    for (int x = 0; x < w; x++) {
        float y = sin((x / 8.0f) + (t / 3.0f)) * (h / 6.0f) + (h / 2.0f);
        oled_write_pixel(x + 0, y + 0, true);
        oled_write_pixel(x + 0, y + 1, true);
        oled_write_pixel(x + 1, y + 0, true);
        oled_write_pixel(x + 1, y + 1, true);
    }
}

static int oled_time_input = 0;
...
void oled_task_user(void) {
    ...
    oled_time_input %= 100;
    oled_clear();
    oled_draw_stuff(oled_time_input);
}
